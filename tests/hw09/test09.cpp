...
struct Marker final {
    ...
    Marker(Marker const&) noexcept
        : state{"copy constructed"}
    {
    }
    Marker(Marker&& other) noexcept
        : state{"move constructed"}
    {
        other.state = "move constructed husk";
    }
    ...
    long long int, float, double, std::string)
{
    ...
    CHECK_UNARY(std::is_same_v<typename Vector<T>::value_type, T>);
    CHECK_UNARY(std::is_unsigned_v<typename Vector<T>::size_type>);
    CHECK_UNARY(std::is_signed_v<typename Vector<T>::difference_type>);
}
...
TEST_CASE("String Copy Assignment")
{
    ...
    CHECK_EQ(w.size(), count);
    CHECK_EQ(v.size(), w.size());
    v[0] = Vector<std::string>::value_type{"New Value!"};
    CHECK_NE(v[0], w[0]);
    CHECK_EQ(w[0], defaultValue);
}
...
TEST_CASE("String Move Constructor")
{
    ...
    for (Vector<std::string>::size_type i{0}; i < w.size(); ++i)
...
TEST_CASE("Iterate from beginning to end (ints)")
{
    SUBCASE("Ranged based for loop")
    {
    ...
    SUBCASE("iterate with begin and end")
    {
        ...
        for (; first != last; ++first) {
            CHECK_EQ(*first, std::distance(v.begin(), first) + 1);
        }
    }

    SUBCASE("change with underlying vector with begin")
    {
        Vector<int> v({1, 2, 3, 4, 5});
        CHECK_EQ(v.size(), 5);
    ...
    SUBCASE("iterate with cbegin and cend")
    {
        Vector<int> v({1, 2, 3, 4, 5});
        CHECK_EQ(v.size(), 5);

        Vector<int>::const_iterator first = v.cbegin();
        Vector<int>::const_iterator last = v.cend();
        for (; first != last; ++first) {
            CHECK_EQ(*first, std::distance(v.cbegin(), first) + 1);
        }
    }

    SUBCASE("reverse iterate with rbegin and rend")
    {
        Vector<int> v({1, 2, 3, 4, 5});
        CHECK_EQ(v.size(), 5);

        Vector<int>::reverse_iterator first = v.rbegin();
        Vector<int>::reverse_iterator last = v.rend();
        auto i = 5;
        for (; first != last; ++first) {
            CHECK_EQ(*first, i);
            --i;
    ...
    SUBCASE("change with underlying vector with rbegin")
    {
        Vector<int> v({1, 2, 3, 4, 5});
        CHECK_EQ(v.size(), 5);
    ...
    SUBCASE("reverse iterate with crbegin and crend")
    {
        Vector<int> v({1, 2, 3, 4, 5});
        CHECK_EQ(v.size(), 5);

        Vector<int>::const_reverse_iterator first = v.crbegin();
        Vector<int>::const_reverse_iterator last = v.crend();
        int i{5};
        for (; first != last; ++first) {
            CHECK_EQ(*first, i);
            --i;
