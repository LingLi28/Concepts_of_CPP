...
`origin` is your personal "fork" of the [base repo](https://gitlab.lrz.de/cppcourse/ws2023/tasks).
...
We will publish new homeworks (and bugfixes :) [**in the base repo**](https://gitlab.lrz.de/cppcourse/ws2023/tasks), which you then merge into [**your repo**](https://gitlab.lrz.de/cppcourse/ws2023) (`your-username_tasks`).
...
You can automate the GitLab login by using an SSH key:
...
3. Now you can clone, fetch and push repos without typing usernames and passwords

Optionally: Configure your computer's `ssh` to use a specific SSH key for logins on GitLab:
...
```
You don't need this config change when you generated the SSH key in step 1 at its default location.
...
use:
  ...
  - alternative: `git clone https://gitlab.lrz.de/cppcourse/ws2023/your-username_tasks.git` if you don't want to use `ssh`, but `https` instead


## Adding Upstream


In order to regularly pull the new assignments and any other changes (bugfixes), you must add the original [base repo](https://gitlab.lrz.de/cppcourse/ws2023/tasks) as `upstream` remote.
...
```console
> origin  git@gitlab.lrz.de:cppcourse/ws2023/your-username_tasks.git (fetch)
> origin  git@gitlab.lrz.de:cppcourse/ws2023/your-username_tasks.git (push)
```
which tells us, that your local git repo knows a remote git repo with the name `origin`, which is your personal GitLab repo.

Now you need to add "[the course organizer's](https://gitlab.lrz.de/cppcourse/ws2023/tasks)" repo so you can get new tasks from us.
...
As we'll be covering modern C++ (C++23) you need a recent compiler, otherwhise your project will not compile.

Recommended compilers are:
- GNU GCC 13 and higher (Linux: install `g++` with your package manager, [Windows](https://www.msys2.org/))
- LLVM Clang 16 and higher (Linux: install `clang` with your package manager)
- (If you really want to, the latest Visual Studio supporting the latest C++ standard)

We will be testing with GCC 13 and Clang 16 on Linux.
...
We use [Doom](https://github.com/doomemacs/doomemacs) [Emacs](https://www.gnu.org/software/emacs/) (unifies the advantages of Vim and Emacs) and [vim](https://www.vim.org/)/[neovim](https://neovim.io/).
...
If you do modify them and get an error message, you must keep your repo history (so no `git force-push`) - you can only ever add new things to your repo history, not delete old history elements.
